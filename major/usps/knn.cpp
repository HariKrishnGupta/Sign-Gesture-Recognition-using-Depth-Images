...
int SEGMENTS=3,SD=1;
int nSV=0;
ofstream fout1("consensus.txt",ios::out);
...
int main(){
	...
	fout.close();
	system("./svm-predict colon50SVM consensus.model outF");*/
	system("./convert consensus.txt > consensusSVM");
	system("./svm-train -q -t 0 consensusSVM");
	system("./svm-predict uspstest100SVM consensusSVM.model outF");
	return 0;
}
...
vector<int> knn(vector< vector<double> >&X,vector<int>&validationData,vector<int>&trainingData,vector<int>&y,vector<int>&SVM){
	...
	for(k=0;k<nValidation;k++){	
		ofstream findex;
		findex.open("tempData.txt",ios::out);
		...
		stringstream oss;
		oss<<"./svm-train"<<" -t 0"<< "tempData.txt"<<"\0";
		...
		*/
		vector<int> index;
        	getDataY(string("index.txt"),index);
        	int templen = index.size();
...
vector<int> knn(vector< vector<double> >&X,vector< vector<double> >&initialCentroid,vector<int>&trainingData,vector<int>&y,vector<int>&SVM){
	...
	for(k=0;k<nValidation;k++){	
		...
		findex.close();
		stringstream oss;
		oss<<"./svm-train -q"<<" -t 0"<< " tempData.txt"<<"\0";
		string tempstr = oss.str();
		system(tempstr.c_str());
		/*
		//cout<<"alpha \n";
...
void getDataY(string filename,vector< int > &index){
	ifstream file;
	file.open(filename.c_str(),ios::in);
	if(file.is_open()){
		while(file.good()){
